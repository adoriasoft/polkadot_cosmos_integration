...
/* pub trait BabeShared {
	fn change_authorities_for_current_epoch(curr_authorities: Vec<(AuthorityId, BabeAuthorityWeight)>) -> bool;
}

impl <T: Trait> BabeShared for Module<T> {
	fn change_authorities_for_current_epoch(curr_authorities: Vec<(AuthorityId, BabeAuthorityWeight)>) -> bool {
		Self::change_authorities(curr_authorities)
	}
} */
...
impl<T: Trait> Module<T> {
	...
	pub fn assign_authority_weight(authority_id: AuthorityId, weight: BabeAuthorityWeight) -> DispatchResult {
...
impl<T: Trait> pallet_session::OneSessionHandler<T::AccountId> for Module<T> {
	...
	{
		let authorities = validators.map(|(_, k)| (k.clone(), <Module<T>>::authorities_weights(k.clone()))).collect::<Vec<_>>();
		...
		where I: Iterator<Item=(&'a T::AccountId, AuthorityId)>
	{
		let authorities = validators.map(|(_account, k)| {
			(k.clone(), <Module<T>>::authorities_weights(k.clone()))
		}).collect::<Vec<_>>();

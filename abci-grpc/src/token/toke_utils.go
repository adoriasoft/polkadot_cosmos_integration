...
func GenerateKeyPairForAccount(account_name string) (string, string) {
	...
	log.Printf("%v, private key: %v", account_name, private_key_encoded)

	log.Printf("{ \"AccountName\": \"%v\", \"PublicKey\": \"%v\"}", account_name, public_key_encoded)
...
func GenerateTransactionMessage(from string, to string, amount int, private_key string, nonce int) {
	...
	log.Printf("{ \"From\": \"%v\", \"To\": \"%v\", \"Amount\": %v, \"Signature\": \"%v\", \"Nonce\": %v}", from, to, amount, signature, nonce)
}
